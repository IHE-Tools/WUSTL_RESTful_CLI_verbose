...
function get_arguments() {
 LOG_FOLDER="$1/qedm/server/condition"
 BASE_URL=$2
...
echo java -jar $JAR	\
  ...
  ./restful_tests/qedm/server/condition.txt	\
  $LOG_FOLDER	\
  $BASE_URL
...
if [ $? != 0 ] ; then
 echo "The script ./restful_tests/scripts/qedm_condition.sh failed to execute java command."
