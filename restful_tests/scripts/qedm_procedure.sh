...
function get_arguments() {
 LOG_FOLDER="$1/qedm/server/procedure"
 BASE_URL=$2
...
echo java -jar $JAR	\
  ...
  ./restful_tests/qedm/server/procedure.txt	\
  $LOG_FOLDER	\
  $BASE_URL
...
if [ $? != 0 ] ; then
 echo "The script ./restful_tests/scripts/qedm_procedure.sh failed to execute java command."
