...
echo java -jar $JAR	\
  REST	\
  ./restful_tests/mcsd/server	\
  ./restful_tests/mcsd/server/query.txt	\
  $LOG_FOLDER	\
  $BASE_URL
...
if [ $? != 0 ] ; then
 echo "The script ./restful_tests/scripts/mcsd.sh failed to execute java command."
