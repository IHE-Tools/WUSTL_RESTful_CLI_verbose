...
function get_arguments() {
 LOG_FOLDER="$1/qedm/server/medication_statement"
 BASE_URL=$2
...
echo java -jar $JAR	\
  REST	\
  ./restful_tests/qedm/server	\
  ./restful_tests/qedm/server/medication_statement.txt	\
  $LOG_FOLDER	\
  $BASE_URL
...
if [ $? != 0 ] ; then
 echo "The script ./restful_tests/scripts/qedm_medication_statement.sh failed to execute java command."
