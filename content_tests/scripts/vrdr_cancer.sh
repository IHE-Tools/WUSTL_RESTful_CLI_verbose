...
function check_arguments() {
 if [ $# != 2 ] ; then
  ...
  echo "           input_file contains JSON representation of cancer case"
  exit 1
 fi
}

function get_arguments() {
 LOG_FOLDER="$1/vrdr/content/cancer"
 INPUT_FILE=$2
...
java -jar $JAR	\
  ...
  ./content_tests/vrdr/testcases-content/cancer_1.txt	\
  $LOG_FOLDER	\
  $INPUT_FILE

if [ $? != 0 ] ; then
 echo "The script ./content_tests/scripts/vrdr_cancer.sh failed to execute java command."
