...
public class TestInstance {
    ...
    public List<ElementTestItem> getTestItems() throws Exception {
        ArrayList<ElementTestItem> rtn = new ArrayList<>();

        List<String> lines = Files.readAllLines(Paths.get(testFolder + "/" + getElements()));
        for (String line: lines) {
            if (line.isEmpty() || line.startsWith("#")) continue;

            String[] tokens = line.split("\\t");
            if (tokens.length == 3) {
                ElementTestItem item = new ElementTestItem(tokens);
                rtn.add(item);
            }
        }
        return rtn;
